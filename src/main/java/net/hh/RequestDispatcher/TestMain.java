...
public class TestMain {
    ...
    public static void fillme() {
        ...
        dp.registerServiceProvider("TEST-A", new ZmqService(
                "tcp://127.0.0.1:60124"));
        dp.setDefaultService(TestRequest.class, "TEST-A");

        dp.registerServiceProvider("TEST-B", new ZmqService(
                "tcp://127.0.0.1:60125"));
        ...
        dp.execute(new TestRequest("Hi From Dispatcher"),
                new Callback<TestReply>(new TestReply()) {

                    @Override
                    public void onSuccess(TestReply reply) {
                        // System.out.println("OUT-A " + reply.serialize());
                        responses.add("A REQUEST" + reply.serialize());
                    }
                });

        dp.execute("TEST-B", new TestRequest("Hi from another one"),
                new Callback<TestReply>(new TestReply()) {
